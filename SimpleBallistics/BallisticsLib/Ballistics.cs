...
namespace SimpleBallistics.BallisticsLib
{
    class Ballistics
    {
        ...
        public const int __BCOMP_MAXRANGE__ = 50001;
        ...
        public static int toMOAClicks(double correction, double distance, double iclicks = 0.25, bool neg = true)
        {
            distance = distance / 100;

            // calc
            double moa = correction / distance;
            double moa_clean = Math.Round(moa, 1);

            // default is using 0.24 or quarter-rounding
            double moa_clicks = Math.Round(moa_clean * 4) / 4;
            int clicks = (int)moa_clicks * 4;
            string scope = "1/4";

            if (iclicks == 0.50)
            {
                moa_clicks = Math.Round(moa_clean * 2) / 2;
                clicks = (int)moa_clicks * 2;
                scope = "1/2";
            }

            if (iclicks == 1)
            {
                moa_clicks = Math.Round(Math.Round(moa_clean, 1));
                clicks = (int)moa_clicks;
                scope = "1";
            }
